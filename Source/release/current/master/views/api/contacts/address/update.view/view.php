...
$addressManager = new ePersonAddress($personID, $addressID);
$status = $addressManager->update($_POST['type'], trim($_POST['address']), trim($_POST['pcode']), trim($_POST['city']), $_POST['country_id']);
if (!$status)
{
	// Show error
	$error = array();
	$error['message'] = "There was an error updating the address";
	return $jsonContent->getReport($error, $allowOrigin = "", $withCredentials = TRUE, $key = "error");
}
...
// Return report
return $jsonContent->getReport($addressList, $allowOrigin = "", $withCredentials = FALSE, $key = "address_list");
//#section_end#
?>