...
if (!empty($accountID))
	$ePerson->initWithAccount($accountID);
...
if (empty($personInfo))
{
	// Show error
	$error = array();
	$error['message'] = "There is no person registered with the given id.";
	return $jsonContent->getReport($error, $allowOrigin = "", $withCredentials = TRUE, $key = "error");
}

// Return report
return $jsonContent->getReport($personInfo, $allowOrigin = "", $withCredentials = FALSE, $key = "info");
//#section_end#
?>