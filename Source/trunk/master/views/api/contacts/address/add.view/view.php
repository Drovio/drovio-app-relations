...
$status = $addressManager->create($_POST['type'], trim($_POST['address']), trim($_POST['pcode']), trim($_POST['city']), $_POST['country_id']);
if (!$status)
{
	// Show error
	$error = array();
	$error['message'] = "There was an error creating the address";
	return $jsonContent->getReport($error, $allowOrigin = "", $withCredentials = TRUE, $key = "error");
}
