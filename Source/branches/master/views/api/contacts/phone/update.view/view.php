...
$phoneID = engine::getVar("phid");
$phoneManager = new ePersonPhone($personID, $phoneID);
$status = $phoneManager->update($_POST['type'], trim($_POST['phone']), $_POST['country_id']);
if (!$status)
{
	// Show error
	$error = array();
	$error['message'] = "There was an error updating the phone";
	return $jsonContent->getReport($error, $allowOrigin = "", $withCredentials = TRUE, $key = "error");
}

// Get updated information
$phoneList = $phoneManager->getAllPhones();

// Return report
return $jsonContent->getReport($phoneList, $allowOrigin = "", $withCredentials = FALSE, $key = "phone_list");
//#section_end#
?>