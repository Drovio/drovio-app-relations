...
if (engine::isPost())
{
	...
	$fm->create($file = "/export/contacts.csv", $csv, $recursive = TRUE);
	
	// Create content
	$appContent->build("", "downloader");
	
	// Create download button with attributes
	$title = appLiteral::get("main.export", "lbl_downloadFile");
	$dlButton = DOM::create("div", $title, "btn_download");
	$actionFactory->setDownloadAction($dlButton, $viewName = "export/downloadFile", $attr = array());
