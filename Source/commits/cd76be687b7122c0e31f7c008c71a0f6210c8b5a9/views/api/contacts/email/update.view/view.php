...
$emailID = engine::getVar("eid");
$mailManager = new ePersonMail($personID, $emailID);
$status = $mailManager->update($_POST['type'], trim($_POST['email']));
if (!$status)
{
	// Show error
	$error = array();
	$error['message'] = "There was an error updating the email address";
	return $jsonContent->getReport($error, $allowOrigin = "", $withCredentials = TRUE, $key = "error");
}

// Get updated information
$mailList = $mailManager->getAllMail();

// Return report
return $jsonContent->getReport($mailList, $allowOrigin = "", $withCredentials = FALSE, $key = "email_list");
//#section_end#
?>