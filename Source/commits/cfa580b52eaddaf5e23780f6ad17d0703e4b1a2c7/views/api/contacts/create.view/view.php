...
$ePerson = new ePerson();
$status = $ePerson->create($_POST['firstname'], $_POST['lastname'], $_POST['middle_name']);
if (!$status)
{
	// Show error
	$info = array();
	$info['status'] = 0;
	$info['message'] = "An error occurred while trying to create the contact relation. Please try again later.";
}
else
{
	// Update person with extra info
	$ePerson->update($_POST['firstname'], $_POST['lastname'], $_POST['middle_name'], $_POST['notes'], $_POST['dob']);
		
	// Successfull update
	$info = array();
	$info['status'] = 1;
	$info['message'] = "Contact relation created successfully!";
	$info['contact_id'] = $ePerson->getPersonID();
	$info['contact_info'] = $ePerson->info();
}

// Return report
return $jsonContent->getReport($info, $allowOrigin = "", $withCredentials = FALSE, $key = "create");
//#section_end#
?>